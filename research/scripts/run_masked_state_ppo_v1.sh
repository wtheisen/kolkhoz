...
RUN_DIR="${RUN_DIR:-research/runs/masked_state_ppo_v1/$(date -u +%Y%m%dT%H%M%SZ)}"
...

{
  printf 'RUN_DIR=%s\n' "$RUN_DIR"
  printf 'OUTPUT=%s\n' "$OUTPUT"
  printf 'ARCHITECTURE=masked-state-mlp\n'
  printf 'EPISODES=%s\n' "$EPISODES"
  printf 'BATCH_SIZE=%s\n' "$BATCH_SIZE"
  printf 'LAYERS=%s\n' "$LAYERS"
  printf 'PPO_EPOCHS=%s\n' "$PPO_EPOCHS"
...
args=(
  -m research.kolkhoz_research.cli masked-state-train
  --output "$OUTPUT"
  --episodes "$EPISODES"
