...
RUN_DIR="${RUN_DIR:-research/runs/masked_state_rnn_ppo_v1/$(date -u +%Y%m%dT%H%M%SZ)}"
...

{
  printf 'RUN_DIR=%s\n' "$RUN_DIR"
  printf 'OUTPUT=%s\n' "$OUTPUT"
  printf 'ARCHITECTURE=masked-state-rnn\n'
  printf 'EPISODES=%s\n' "$EPISODES"
  printf 'BATCH_SIZE=%s\n' "$BATCH_SIZE"
...
args=(
  -m research.kolkhoz_research.cli masked-state-rnn-train
  --output "$OUTPUT"
  --episodes "$EPISODES"
