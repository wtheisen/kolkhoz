...
class AdminControlApplication:
    def __init__(self, *, auth: object, admin_user_ids: frozenset[str],
                 restart: object, cooldown_seconds: float = 300,
    ...
    async def __call__(self, scope: dict[str, object], receive: object, send: object) -> None:
        ...
        if method != "POST" or path != "/admin/control/restart":
            ...
            return
        headers = {bytes(k).decode().lower(): bytes(v).decode() for k, v in scope.get("headers", [])}
        try:
            ...
            if headers.get("x-kolkhoz-restart-confirm") != "restart":
                raise ServerError(HTTPStatus.BAD_REQUEST, "restart confirmation required")
            with self._lock:
                now = self.clock()
                if now - self._last_restart < self.cooldown_seconds:
                    raise ServerError(HTTPStatus.TOO_MANY_REQUESTS, "restart cooldown active")
                self.restart()
                self._last_restart = now
            logging.warning("admin restarted kolkhoz-greenfield.service", extra={"admin_user_id": user_id})
    ...
    async def _respond(send: object, status: int, body: object) -> None:
        ...
        await send({"type": "http.response.start", "status": int(status), "headers": [(b"content-type", b"application/json"), (b"content-length", str(len(encoded)).encode())]})
...
def create_admin_control_application() -> AdminControlApplication:
    ...
    admins = frozenset(
        value.strip() for value in os.environ.get("KOLKHOZ_ADMIN_USER_IDS", "").split(",")
        if value.strip()
    )
    if auth is None or not admins:
        raise RuntimeError("admin control requires Supabase auth and an admin allowlist")
    return AdminControlApplication(
        auth=auth, admin_user_ids=admins, restart=_restart_service,
        cooldown_seconds=float(os.environ.get("KOLKHOZ_RESTART_COOLDOWN_SECONDS", "300")),
    )
