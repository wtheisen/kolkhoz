...
| Remaining limitation | Current evidence | Required closure |
|---|---|---|
| Flutter still subscribes to legacy Supabase `game_updates`; the greenfield WebSocket currently wakes clients only for action revisions. | The WebSocket authenticates bearer plus seat token, sends a viewer-safe initial state, performs durable revision catch-up, suppresses duplicates, and fails over with close `1013`. `FLUTTER_REALTIME_MIGRATION.md` records the exact client contract and why dual-writing the legacy tables is unsafe. | Move Flutter to the WebSocket contract while retaining its connected HTTP correctness poll for non-action lobby/reaction changes. Add a durable metadata cursor before removing that poll. |
