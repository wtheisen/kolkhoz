...
app in `ios/KolkhozSwiftUI/`. The older browser/boardgame.io implementation still
exists in the repo, but when rules or behavior differ, follow the C engine and its Swift
adapter.
...
app runtime paths. The web build in `docs/` is generated by `npm run build` from the
legacy web source and may not match current rule behavior.
